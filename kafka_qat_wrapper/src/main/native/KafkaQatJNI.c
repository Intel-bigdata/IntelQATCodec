#include "com_intel_qat_codec_io_nativ_QatNative.h"
#include "qatzip.h"
...
JNIEXPORT void JNICALL Java_com_intel_qat_codec_io_nativ_QatNative_init
  (JNIEnv *env, jclass cls) {
    ...
    if (qat_wrapper_context->maxCompressedLength == NULL) {
        THROW(env, "java/lang/UnsatisfiedLinkError", "Failed to load qzMaxCompressedLength");
        return;
    }
}


JNIEXPORT jint JNICALL Java_com_intel_qat_codec_io_nativ_QatNative_compress
  (JNIEnv *env, jobject jobj, jbyteArray src, jint srcOff, jint srcLen, jbyteArray des, jint desOff){
...
JNIEXPORT jint JNICALL Java_com_intel_qat_codec_io_nativ_QatNative_decompress
  (JNIEnv *env, jobject jobj, jbyteArray src, jint srcOff, jint srcLen, jbyteArray des, jint desOff){
	jbyte *in = (*env)->GetByteArrayElements(env, src, 0);
    if (in == NULL) {
...
JNIEXPORT jint JNICALL Java_com_intel_qat_codec_io_nativ_QatNative_maxCompressedLength
  (JNIEnv *env, jobject jobj, jint length){

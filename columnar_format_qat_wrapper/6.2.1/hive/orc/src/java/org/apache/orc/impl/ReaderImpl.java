...
public class ReaderImpl implements Reader {
  ...
  private static final int DIRECT_BUFFER_SIZE = 512 * 1024;
  ...
  * Constructor that let's the user specify additional options.
  ...
  public ReaderImpl(Path path, OrcFile.ReaderOptions options) throws IOException {
    ...
    if (fileMetadata != null) {
      ...
      this.codec = WriterImpl.createCodec(compressionKind, DIRECT_BUFFER_SIZE);
  ...
  public static OrcTail extractFileTail(ByteBuffer buffer, long fileLength, long modificationTime)
    ...
    CompressionCodec codec = WriterImpl
        .createCodec(CompressionKind.valueOf(ps.getCompression().name()), DIRECT_BUFFER_SIZE);
  ...
  protected OrcTail extractFileTail(FileSystem fs, Path path,
    ...
    try {
      ...
      codec = WriterImpl.createCodec(CompressionKind.valueOf(ps.getCompression().name()), DIRECT_BUFFER_SIZE);
